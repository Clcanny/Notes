...
`rec->run`也仅仅是调用传入给`create_context1`作为参数的lambda函数

我们传入的lambda函数会有一个返回值，是什么呢？不妨以boost_test.cpp作为例子：

![30](30.jpg)

是main函数的执行环境sink

也即：`rec->run()`的结果是另外一个不相关的continaution的执行环境（栈顶指针）

注意，如果`fn`通过`yield`切换，并不会返回；也就是说，收到`rec->run()`的返回值一定是在`fn`执行完毕之后

![31](31.jpg)

既然`fn`已经返回（再也不会执行），自然可以销毁它的执行环境



call/cc的故事到这里就结束了，至此我们也看清楚boost.context的大致原理

只不过到现在的文章，都是用一个初学者的角度（一步步深挖）来写的，虽然展示了如何一步步往下走的过程，却少了些许逻辑／脉络不清晰；所以，之后会推出总结性的文章，更有条理地看待libgo以及它所依赖的boost.context