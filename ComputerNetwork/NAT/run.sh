...
docker exec router1 bash -c "iptables -P INPUT ACCEPT"
docker exec router1 bash -c "iptables -P FORWARD ACCEPT"
docker exec router1 bash -c "iptables -P OUTPUT ACCEPT"
# docker exec router1 bash -c "iptables -t nat -A POSTROUTING -s 192.168.2.2 -o eth3 -j SNAT --to-source 210.28.120.3"
# docker exec router1 bash -c "iptables -t nat -A POSTROUTING -s 192.168.2.3 -o eth3 -j SNAT --to-source 210.28.120.4"
# docker exec router1 bash -c "iptables -t nat -A POSTROUTING -s 192.168.2.130 -o eth3 -j SNAT --to-source 210.28.120.5"
docker exec router1 bash -c "iptables -t nat -A PREROUTING -d 210.28.120.3 -i eth3 -j DNAT --to-destination 192.168.2.2"
docker exec -d router1 bash -c "tshark -w test.tshark"
...
docker exec -d router2 bash -c "tshark -w test.tshark"
...
docker exec -d container1 bash -c "tshark -w test.tshark"
...
docker exec -d container2 bash -c "tshark -w test.tshark"
...
docker exec -d container3 bash -c "tshark -w test.tshark"
...
docker exec container4 bash -c "ping 210.28.120.3 -c 4" | tail -1

sleep 5
docker cp router1:/test.tshark router1.tshark
docker cp container1:/test.tshark container1.tshark
